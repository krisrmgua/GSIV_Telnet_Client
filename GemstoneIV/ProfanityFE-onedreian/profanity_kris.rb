...
if Opts.file
    SETTINGS_FILENAME           = Settings.file(Opts.file)
else
				...
				if current_stream
					if current_stream == 'thoughts'
						if text =~ /^\[.+?\]\-[A-z]+\:[A-Z][a-z]+\: "|^\[server\]\: /
							current_stream = 'lnet'
                                                ...
                                                text1 = text
                                                ddate = " #{Time.now.strftime('%l:%M%P').sub(/^0/, '')}"
                                                text = text1 + ddate
					end
					if window = stream_handler[current_stream]
						if current_stream == 'death'
							...
							if text =~ /^\s\*\s(The death cry of )?([A-Z][a-z]+)\s?(?:just bit the dust!|has gone to feed the fishes in the Elemental Confluence!|just sank to the bottom of the The Great Western Sea!|failed to bring a shrubbery to the Night at the Academy!|was just defeated in the Arena of the Abyss!|just died on the Great Western Sea!|just got squashed!|just perished defending a fortress within Reim!|is off to a rough start!  (He|She) has gone to feed the fishes!|is off to a rough start!  (He|She) just turned his last page!|is off to a rough start!  (He|She) is dust in the wind!|just sunk to the bottom of the Tenebrous Cauldron!|is dust in the winds of the Elemental Confluence!|\'s life on land appears to be as rough as (his|her) life at sea.|just sunk the Tenebrous Cauldron!|just turned (his|her) last page in the Elemental Confluence!|has been vaporized!|has gone to feed the fishes!|is now fish food for the fauna of Caligos Isle!|was just defeated in Duskruin Arena!|just bit the dust... in Black Swan Castle!|just bit the dust... somewhere around Bloodriven Village!|failed within the Bank at Bloodriven!|\'s flame just burnt out in the Sea of Fire!|is off to a rough start!  (He|She) is going home on (his|her) shield!|is going home on (his|her) shield!|may just be going home on (his|her) shield!|just punched a one-way ticket!|just punched a one-way ticket to the Elemental Confluence!|just lost (his|her) way somewhere in the Settlement of Reim!|just gave up the ghost!|is dust in the wind!|is off to a rough start!  (He|She) was just put on ice!|was just put on ice in the Elemental Confluence!|just sank to the bottom of the Tenebrous Cauldron!|has just returned to Gosaena!|is off to a rough start!  (He|She) just bit the dust!|just bit the dust in the Elemental Confluence!|just lost (his|her) way somewhere in the Settlement of Reim!|is six hundred feet under!|was just put on ice!|just took a long walk off of a short pier!|just took a long walk off of a short pier in the Elemental Confluence!|is off to a rough start!  (He|She) just took a long walk off of a short pier!|was just incinerated!|just turned (his|her) last page!|echoes in your mind!)/
								front_count = 3
								front_count += 17 if $1
								name = $2
								text1 = text
								...
								line_colors.delete_if { |h| h[:start] >= h[:end] }
                                                                if text1 =~/was just put on ice!/
									h = {
										:start => (name.length+1),
										:end => text.length,
										:fg => '00FFF0',
									}
								elsif text1 =~/just bit the dust!/
                                                                        h = {
                                                                                :start => (name.length+1),
                                                                                :end => text.length,
                                                                                :fg => '70C63E',
                                                                        }
                                                                elsif text1 =~/has gone to feed the fishes!/
                                                                        h = {
                                                                                :start => (name.length+1),
                                                                                :end => text.length,
                                                                                :fg => '88aaff',
                                                                        }
                                                                elsif text1 =~/off of a short pier!/
                                                                        h = {
                                                                                :start => (name.length+1),
                                                                                :end => text.length,
                                                                                :fg => '7A7A7A',
